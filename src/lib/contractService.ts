...
export const PORTFOLIO_CONTRACT_ADDRESS = (import.meta.env
  .VITE_PORTFOLIO_CONTRACT_ADDRESS ||
...
export const updateAllocations = async (allocations: Allocation[]) => {
  ...
  try {
    console.log(
      "Starting direct contract interaction with allocations:",
      allocations
    );
    ...
    try {
      const gasEstimate = await contract.updateAllocations.estimateGas(
        categories,
        percentages
      );
...
export function useIsContractOwner() {
  ...
  const isOwner =
    address && data
      ? address.toLowerCase() === (data as string).toLowerCase()
      : false;
...
export function useUpdateAllocations() {
  ...
  const { writeContractAsync, isPending, error, isSuccess, data, status } =
    useWriteContract();
  ...
  const updateAllocationsFn = async (allocations: Allocation[]) => {
    ...
    if (
      ...
      throw new Error(
        "Invalid contract address. Please check your environment variables."
      );
    }
    ...
    if (!isOwner) {
      ...
      throw new Error(
        "You are not the owner of this contract. Only the owner can update allocations."
      );
    }
...
export function getExplorerUrl(hash: string) {
  const explorerUrl =
    import.meta.env.VITE_IOTA_EXPLORER_URL ||
